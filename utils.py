...
rg = rouge.Rouge()
def rouge_n(preds, trues):
    preds = [" ".join(p) for p in preds]
    trues = [" ".join(t) for t in trues]
    scores = rg.get_scores(preds, trues)
    return {
        "macro_rouge1": {key: sum([x["rouge-1"][key] for x in scores]) / len(scores) for key in ["p", "r", "f"]},
        "macro_rouge2": {key: sum([x["rouge-2"][key] for x in scores]) / len(scores) for key in ["p", "r", "f"]},
        "macro_rougeL": {key: sum([x["rouge-l"][key] for x in scores]) / len(scores) for key in ["p", "r", "f"]},
    }
